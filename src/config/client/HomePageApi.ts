...
const HOME_PAGE_ENDPOINTS = {
  BOOKS_BY_CATEGORY_ID: (categoryId: string) =>
  ...
  SEARCH_BOOKS: (searchType: string, searchKey: string) =>
    `/api/books/search?type=${searchType}&key=${searchKey}`,
...
class HomePageApi {
  homePage = {
    ...
    getCategoriesWithBooks: async () => {
      return await fetcher.get(HOME_PAGE_ENDPOINTS.CATEGORIES_WITH_BOOKS);
    },
    ...
    getShoppingCardBook: async () => {
      return await authFetcher.get(HOME_PAGE_ENDPOINTS.SHOPING_CARD_BOOKS);
    },
    ...
    reserveBooks: async (bookId: string) => {
      return await authFetcher.post(HOME_PAGE_ENDPOINTS.RESERVE_BOOKS(bookId));
    },
    ...
    getFacutlyWithDepartments: async () => {
      return await fetcher(HOME_PAGE_ENDPOINTS.GET_FACULTY_WITH_DEPARTMENTS);
    },
    getHomeData: async () => {
      return await fetcher(HOME_PAGE_ENDPOINTS.GET_HOME_DATA);
    },
    getProfile: async () => {
      return await authFetcher.get(HOME_PAGE_ENDPOINTS.VIEW_PROFILE);
    },
  };
}
