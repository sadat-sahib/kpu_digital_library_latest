...
const BorrowsReport: React.FC = () => {
  ...
  const handleFacultyChange = (e: ChangeEvent<HTMLSelectElement>) => {
    ...
    // Update query with selected faculty
    fetchBorrowedBooks(facultyId.toString());
  };
  ...
  const handleDepartmentChange = (e: ChangeEvent<HTMLSelectElement>) => {
    ...
    // Update query with selected department
    fetchBorrowedBooks(selectedFaculty?.id.toString(), deptId);
  };

  // Fetch borrowed books with optional filters
  const fetchBorrowedBooks = (facultyId?: string, departmentId?: string) => {
    setLoading(true);
    
    let url = '/api/books/borrowed';
        ...
        <html>
          ...
          <body>
            <div class="report-header">
              ...
              <h1>گزارش کتاب های امانت گرفته شده</h1>
            ...
            <table>
              ...
              <tbody>
                ${borrowedBooks.length > 0 ? 
                  ...
                  `}).join('') : `
                    <tr>
                      <td colspan="7" style="text-align: center;">هیچ کتاب امانت گرفته شده ای یافت نشد</td>
                    </tr>
                  `}
              </tbody>
            </table>
    ...
    <div ref={reportRef} className="p-4 max-w-6xl mx-auto">
      <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <table className="min-w-full divide-y divide-gray-200 print:table print:w-full">
              <thead className="bg-gray-50 print:bg-gray-100">
                <tr>
                  ...
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider print:px-2 print:py-2 print:text-sm">شماره محصل</th>
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider print:px-2 print:py-2 print:text-sm">بخش</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {borrowedBooks.length > 0 ? (
                  borrowedBooks.map((book) => {
                    const dueDateStatus = getDueDateStatus(book.dueDate);
                    return (
                      <tr key={book.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 print:px-2 print:py-2 print:text-sm">{book.studentName}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 print:px-2 print:py-2 print:text-sm">{book.studentId}</td>
                  ...
                  <tr>
                    <td colSpan={7} className="px-6 py-4 text-center text-sm text-gray-500 print:px-2 print:py-2">
                      {selectedDepartment ? 'هیچ کتاب امانت گرفته شده ای یافت نشد' : 'لطفاً یک بخش را برای مشاهده کتاب ها انتخاب کنید'}
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="mt-6 flex justify-end no-print">
          <button
            onClick={handlePrint}
            disabled={!selectedDepartment || borrowedBooks.length === 0}
            className={`bg-blue-600 text-white font-medium py-2 px-4 rounded-md flex items-center gap-2 
              ${(!selectedDepartment || borrowedBooks.length === 0) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700'}`}
          >
