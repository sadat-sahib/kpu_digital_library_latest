...
//         queryClient.invalidateQueries({ queryKey: ["shoppingCart"] });
//       },
//       // REMOVE the onError here - it's already handled in the mutation hook
//     });
//   };
...
//                         className="flex items-center justify-center w-full md:w-24 text-sm px-4 py-2"
//                         onClick={() => handleReserve(book.id)}
//                         disabled={reservMutation.isPending}
//                       >
//                         {reservMutation.isPending ? "در حال ثبت..." : "ثبت درخواست"}
//                       </Button>
//                       <Button
//                         variant="destructive"
//                         className="flex items-center justify-center w-full md:w-24 text-sm px-4 py-2"
...
const BookCard: React.FC<{ book: Book }> = ({ book }) => {
  ...
  const handleReserve = () => {
    reserveMutation.mutate(undefined, { // No need to pass bookId since it's already in the hook
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ["shoppingCart"] });
      },
    });
    ...
    <Card key={book.id} className="shadow-lg w-full max-w-[250px] mx-auto">
      ...
      <CardContent className="p-3">
        ...
        <div className="flex flex-col md:flex-row md:justify-between gap-2 mb-3">
          ...
          >
            {reserveMutation.isPending ? "در حال ثبت..." : "ثبت درخواست"}
          </Button>
          <Button
            variant="destructive"
            className="flex items-center justify-center w-full md:w-24 text-sm px-4 py-2"
